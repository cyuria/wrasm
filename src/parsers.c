...
const struct parser_t rv64s[] = {
    ...
    /*
      {"mv", 4, &gen_math, 0x0, 0, 0},
      {"not", 4, &gen_math, 0x1, 0, 0},
      {"neg", 4, &gen_math, 0x2, 0, 0},
      {"negw", 4, &gen_math, 0x3, 0, 0},
  ...
  {                                                                            \
    ...
    if (memcmp(args.type, required, sizeof(required))) {                       \
      logger(ERROR, error_invalid_syntax,                                      \
             "Incorrect arguments for instruction %s", parser.name);           \
      return error_bytecode;                                                   \
    }                                                                          \
  }

static int32_t get_relative_address(struct symbol_t *sym, size_t position) {
  const size_t labelpos = calc_fileoffset(
      (struct sectionpos_t){.section = sym->section, .offset = sym->value});
...
struct bytecode_t gen_nop(struct parser_t parser, struct args_t args,
  ...
  return gen_itype(
      parser,
      (struct args_t){{arg_register, arg_register, arg_immediate}, {0, 0, 0}},
      position);
}
...
struct bytecode_t gen_load_short(struct parser_t parser, struct args_t args,
  ...
  if (parser.opcode)
    value = (uint32_t)get_relative_address((void *)args.arg[1], position);
  logger(DEBUG, no_error, " | value: %.08x", value);

  struct bytecode_t upper =
      gen_utype((struct parser_t){"internal", RV64I_SIZE, NULL,
  ...
  struct bytecode_t lower =
      gen_itype((struct parser_t){"internal", RV64I_SIZE, NULL, OP_OPI, 0x0, 0},
...
struct bytecode_t gen_rtype(struct parser_t parser, struct args_t args,
  ...
  struct bytecode_t res = {.size = RV64I_SIZE, .data = xmalloc(RV64I_SIZE)};

  *(uint32_t *)&(res.data) =
      (parser.opcode | (args.arg[0] << 7) | (args.arg[1] << 15) |
       (args.arg[2] << 20) | (parser.funct1 << 12) | (parser.funct2 << 25));
  return res;
}
...
struct bytecode_t gen_itype(struct parser_t parser, struct args_t args,
  ...
  *(uint32_t *)res.data = (parser.opcode & 0x7F) | ((args.arg[0] & 0x1F) << 7) |
                          ((parser.funct1 & 0x7) << 12) |
                          ((args.arg[1] & 0x1F) << 15) |
                          ((args.arg[2] & 0xFFF) << 20);

  logger(DEBUG, no_error, " | Machine Code: %.08x", *(uint32_t *)res.data);
...
struct bytecode_t gen_utype(struct parser_t parser, struct args_t args,
  ...
  *(uint32_t *)res.data =
      (parser.opcode | (args.arg[0] << 7) | (args.arg[1] & 0xFFFFF000));
  return res;
}
...
struct bytecode_t gen_fence(struct parser_t parser, struct args_t args,
  ...
  *(uint32_t *)res.data =
      (parser.opcode | (parser.funct1 << 12) | (((uint32_t)args.arg[0]) << 20));
  return res;
}
